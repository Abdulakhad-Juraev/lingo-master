...
use common\modules\university\models\search\DirectionSearch;
...
class DirectionController extends BaseController
{
    ...
    {
        $model = new Direction(['status' => Direction::STATUS_ACTIVE]);
    ...
    {
        $model = Direction::find()->andWhere(['id' => $id])->one();
        if ($model == null) {
            not_found();
        }
        return $model;
    ...
    public function actionChange($id)
    {
        ...
        if ($model->status == Direction::STATUS_ACTIVE) {
            $model->status = Direction::STATUS_INACTIVE;
        } else {
