...
class IeltsQuestionsListeningController extends BaseController
{
    public function actionIndex($id)
    {
        ...
        $query = IeltsQuestions::find()
            ...
            ->andWhere(['group_type' => IeltsQuestions::TYPE_LISTENING_GROUP]);
    ...
    public function actionCreate($id)
    {
        ...
        $model = new IeltsQuestions([
            ...
            'group_type' => IeltsQuestions::TYPE_LISTENING_GROUP
        ]);

        $modelsOption = [new IeltsOptions()];
        $model->scenario = IeltsQuestions::SCENARIO_LISTENING;
        if ($model->load(Yii::$app->request->post())) {
            if ($model->insertMultiple($modelsOption)) {
                return $this->redirect(['ielts-questions-listening/index', 'id' => $ielts_question_group->id]);
            }
        }
    ...
    public function actionUpdate($id)
    {
        ...
        $model->group_type = IeltsQuestions::TYPE_LISTENING_GROUP;
        $exam = $model->exam;
        $modelsOption = $model->ieltsOptions;
        if ($model->load(Yii::$app->request->post())) {
            if ($model->updateMultiple($modelsOption)) {
                return $this->redirect(['ielts-questions-listening/index', 'id' => $model->question_group_id]);
            }
        }
